...
const handleNewEmployee = async (req: Request, res: Response) => {
  const { employeeUser, password, email, employeeRoles, organization } =
    req.body
  if (!employeeUser || !password || !email || !employeeRoles || !organization)
  ...
  try {
    ...
    const updateEmployee = await User.findOneAndUpdate(
      { _id: foundEmployee._id },
      {
        ...
        organization:
          foundOrganization.map(organization => organization._id) ||
          foundEmployee.organization
      },
      { new: true }
    )
